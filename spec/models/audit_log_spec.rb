...
RSpec.describe AuditLog, type: :model do
 ...
 describe "validations" do
   ...
   it "should have a start date equal to 2 days prior" do
     new_audit_log = AuditLog.create(user_id: User.last.id, status: 0, start_date: (Date.today - 2.days))
     expect(new_audit_log.start_date).to eq(Date.today - 2.days)
   end
 end
end
